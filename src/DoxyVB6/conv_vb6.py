...
class Vb6Parser(AbstractCodeParser):
    ...
    def _parse_cls(self, code_lines: List[str]) -> CodeElement:
        ...
        lastProperty: CodeElement | None = None

        s = ""
        lineContinue = False
        for ln in code_lines:
            # Processes line continuation by combining lines ending with ` _`.
            if (not _has_comment(ln)) and ln.endswith(" _\n"):
                if lineContinue:
                    s += ln[:-2]
                else:
                    s = ln[:-2]
    ...
    def _get_accessibility(
        ...
        raise CannotParseException(f"Unknown accessibility: {accessibility}")
    ...
    def _find_property_get(
        self, s: str, target_elem: CodeElement, prev_elem: CodeElement | None
    ) -> tuple[bool, CodeElement, CodeElement | None]:
