...
class Test_conv_vb6:
    ...
    @staticmethod
    def test_parse_strips_comment_after_string_literal_with_apostrophe():
        # Arrange
        vb6_parser = conv_vb6.Vb6Parser(conv_vb6.Vb6ModuleType.STANDARD)

        # Act
        actual = vb6_parser.parse(
            [
                'Attribute VB_Name = "CommentProbe"\n',
                'Public Const Selector As String = "iframe[name=\'right\']" \' comment\n',
            ]
        )

        # Assert
        target_elem = actual.root.children[1].children[0].children[0]
        assert target_elem.name == "Selector"
        assert target_elem.value == "iframe[name='right']"

    @staticmethod
    def test_main_processes_long_json_string_literal_without_timeout(tmp_path):
        # Arrange
        src_file = tmp_path / "LongJson.bas"
        json_body = "{" + ",".join(
            f'""key{i}"":""value{i}""' for i in range(40)
        ) + "}"
        src_file.write_text(
            "Attribute VB_Name = \"LongJson\"\r\n"
            "Option Explicit\r\n"
            "Public Sub Test()\r\n"
            f"    Dim body As String: body = \"{json_body}\"\r\n"
            "End Sub\r\n",
            encoding="cp932",
        )
        env = os.environ.copy()
        ...
        actual = subprocess.run(
            [sys.executable, "-m", "DoxyVB6", str(src_file)],
            capture_output=True,
        ...
        assert "public void Test()" in actual.stdout
